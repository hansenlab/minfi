# TODO: Could do this in rowwise chunks to avoid realizing all of M in memory
dmpFinder <- function(dat, pheno, type = c("categorical", "continuous"),
    ...
    if (is(dat, "MethylSet")) {
        ...
        # TODO: namespace clash between DelayedArray::type() and
        #       Biostrings::type()?
        stopifnot(DelayedArray::type(dat) %in% c("integer", "double"))
        M <- dat
    }
    n <- length(pheno)
    if (n != ncol(M))
        stop("length of pheno does not equal number of samples")
    if (type == "categorical") {
        pheno <- factor(as.character(pheno))
        design <- model.matrix(~pheno)
        # NOTE: lmFit() realizes M in memory as an array
        fit <- lmFit(M, design)
        if (shrinkVar) {
            ...
            # NOTE: lmFit() realizes M in memory as an array
            fit1 <- lmFit(M)
            RSS1 <- rowSums((M - DelayedArray(fitted(fit1))) ^ 2)
            RSS <- rowSums((M - DelayedArray(fitted(fit))) ^ 2)
