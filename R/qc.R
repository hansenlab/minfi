# TODO: Calling `qcReport(rgSet)` results in a PDF where no data are plotted for the first 2
#       pages. I think it's because sampGroups is NULL by default. This is broken in the
#       version of minfi available from Bioconductor, not just in this branch
...
densityPlot <- function(dat, sampGroups=NULL, main="", xlab="Beta", pal=brewer.pal(8, "Dark2"), xlim, ylim, add=TRUE, legend=TRUE, ...) {
    ...
    # NOTE: Have to ensure data are in memory as an ordinary vector before computing density
    # TODO: Could speed up using bplapply()
