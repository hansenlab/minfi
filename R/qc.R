...
controlStripPlot <- function(rgSet, controls=c("BISULFITE CONVERSION I", "BISULFITE CONVERSION II"),
    ...
    for (controlType in controls) {
        ...
        ctlR <- melt(as.matrix(ctlWide), varnames=c("address", "sample"))

        ## Green channel
        ctlWide <- log2(g[ctrlAddress,,drop=FALSE])
        if (!is.null(sampNames)) colnames(ctlWide) <- sampNames
        ctlG<- melt(as.matrix(ctlWide), varnames=c("address", "sample"))
...
# HDF5: Loads `dat[idx, ]` into memory
densityBeanPlot <- function(dat, sampGroups=NULL, sampNames=NULL, main=NULL, pal=brewer.pal(8, "Dark2"), numPositions=10000) {
    ...
    if(is.null(numPositions)) {
        idx <- seq_len(dim(dat)[1])
    } else {
        # NOTE: sort() improves access if b is HDF5Array-backed
        idx <- sort(sample(nrow(b), numPositions))
    }
    x <- melt(as.matrix(b[idx, ]), varnames=c("cpg", "sample"))
    o <- order(colnames(b))
...
densityPlot <- function(dat, sampGroups=NULL, main="", xlab="Beta", pal=brewer.pal(8, "Dark2"), xlim, ylim, add=TRUE, legend=TRUE, ...) {
    ...
    # NOTE: Realizes as a vector one sample at a time
    d <- lapply(seq_len(ncol(b)), function(j) {
        density(as.vector(b[, j]), na.rm = TRUE)
    })
    names(d) <- colnames(b)
