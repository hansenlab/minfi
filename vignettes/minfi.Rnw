...
conditions).  This is different from differentially methylated regions (DMRs) which imply
...
\Rfunction{read.450k.sheet}.  This function has a couple of attractive bells and whistles.  Let us look
at the output
...
\Rfunction{read.450k.exp} function is a column named \Rcode{Basename}.  Typically, such an object would
...
Finally, \minfi\ contains a file-based parser: \Rcode{read.450k}.  The return object represents the
