...
If you are using \Biocpackage{minfi} in a publication, please cite \citet{minfi}.  The SWAN
normalization method is described in \citep{Maksimovic:2012}.  The code for parsing Illumina IDAT
files is from the \Biocpackage{illuminaio} described in \citet{illuminaio}.
...
%% \Rcode{read.450k.exp} function is a column named \Rcode{Basename}.  Typically, such an object would
...
\begin{figure}
  \includegraphics[width=.8\textwidth]{minfiClasses.pdf}
